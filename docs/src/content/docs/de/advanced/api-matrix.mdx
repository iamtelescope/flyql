---
title: Sprachübergreifende API-Matrix
description: Vollständige Zuordnung der öffentlichen API-Funktionen und -Typen in Go, Python und JavaScript
---
...
| Value types | `types.Integer`, etc. | `LiteralKind.INTEGER`, etc. | `LiteralKind.INTEGER`, etc. |
...
Der [Tokenizer](/de/advanced/tokenize/) ist ein gemeinsames Primitiv, das die zeichenweise Ausgabe des Parsers zu typisierten Tokens mit `{text, type, start, end}` zusammenfasst. Nutze ihn, um eigene Highlighter oder Formatter auf Basis von FlyQL zu bauen.

| Operation | Go | Python | JavaScript |
|-----------|------|--------|------------|
| Tokenize query | `Tokenize(text, "query")` | `tokenize(text)` | `tokenize(text)` |
| Tokenize columns | _(N/A)_ | _(wirft `ValueError`)_ | `tokenize(text, { mode: 'columns' })` |
| **Types** | | | |
| Token | `Token` (plain struct) | `Token` (frozen dataclass) | plain object |

Der Columns-Modus ist nur in JavaScript verfügbar. Python wirft `ValueError` und Go gibt einen nicht-nil Fehler mit der Meldung `columns mode is only available in the JavaScript package` zurück.
...
| Transformer type | `Type` | `Type` | `Type` |
...
**Python** — `from flyql import parse, bind_params, Node, Expression, Parameter, FunctionCall, Key, Column, ColumnSchema, Operator, Range, Diagnostic, diagnose, LiteralKind`

**JavaScript** — `import { parse, bindParams, Node, Expression, Parameter, FunctionCall, Key, Column, ColumnSchema, Operator, Range, LiteralKind } from 'flyql'`
