...
- **Code-point offsets.** `start`/`end` are Unicode code-point offsets in every language — each character, ASCII or not (including astral / non-BMP characters such as emoji), advances the offset by exactly one. Offsets are identical across Python, Go, and JavaScript for all input; they are not byte offsets (Go) or UTF-16 code-unit offsets (JavaScript). Convert to your editor's coordinate system on your end if it differs.
