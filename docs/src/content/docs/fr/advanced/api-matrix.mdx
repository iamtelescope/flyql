...
Le [tokeniseur](/fr/advanced/tokenize/) est une primitive partagée qui regroupe la sortie caractère-par-caractère du parser en tokens typés de forme `{text, type, start, end}`. Utilise-le pour construire des colorisateurs ou formateurs personnalisés au-dessus de FlyQL.

| Operation | Go | Python | JavaScript |
|-----------|------|--------|------------|
| Tokenize query | `Tokenize(text, "query")` | `tokenize(text)` | `tokenize(text)` |
| Tokenize columns | _(N/A)_ | _(lève `ValueError`)_ | `tokenize(text, { mode: 'columns' })` |
| **Types** | | | |
| Token | `Token` (plain struct) | `Token` (frozen dataclass) | plain object |

Le mode columns est réservé à JavaScript. Python lève `ValueError` et Go renvoie une erreur non-nil avec le message `columns mode is only available in the JavaScript package`.
