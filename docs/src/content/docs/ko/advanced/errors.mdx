---
title: 에러 레퍼런스
description: flyql 진단과 파서 에러를 읽는 방법
---
...
flyql은 모든 진단 코드와 파서 errno의 단일 레지스트리를 제공합니다. 이 레지스트리는 Python, JavaScript, Go를 동기화 상태로 유지하는 단일 진실 공급원이며, 소비자가 보는 모든 `Diagnostic` 또는 파서 오류는 그것에 대한 선택적 참조를 가집니다.
