...
| String（取反） | `message not has 'error'` | `` (`message` IS NULL OR INSTR(`message`, 'error') = 0) `` |
| Array | `tags has 'web'` | `` array_contains(`tags`, 'web') `` |
| Array（取反） | `tags not has 'web'` | `` NOT array_contains(`tags`, 'web') `` |
| Map | `metadata has 'key'` | `` array_contains(map_keys(`metadata`), 'key') `` |
| Map（取反） | `metadata not has 'key'` | `` NOT array_contains(map_keys(`metadata`), 'key') `` |
