...
`parse()`는 AST를 나타내는 `root` 노드를 가진 `ParseResult` 객체를 반환합니다. 입력이 잘못되면 `ParserError`를 throw합니다.
...
| `flyql/tokenize` | 순수 [토크나이저](/ko/advanced/tokenize/) 프리미티브 — 커스텀 포매터를 위한 평면 타입 토큰 |
