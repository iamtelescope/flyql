...
The suggestion panel is portalled to `document.body` and stacks at `--flyql-panel-z-index` (default `100`). If the editor sits inside a host overlay that outranks it, either raise that variable or pass `panelContainer` — a selector or element — so the panel renders inside the overlay and inherits its stacking context. The portalled node carries a `data-flyql-panel` attribute for outside-click checks.
