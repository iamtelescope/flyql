...
| `inputType` | TransformerType | Expected input type (`string`, `int`, `float`, `bool`, `array`), or `any` for a wildcard input that accepts any upstream type — see [Any-input transformers](#any-input-transformers) |
...
## Any-input transformers

A transformer can declare `inputType = Type.Any` (string value `"any"`) to accept any upstream column or transformer output type. The motivating use case is type-agnostic operations — for example, a `type` transformer that returns the column's type name regardless of what that type is.

Rules:

- `Any` is valid **only** on `inputType`. Declaring `Any` as `outputType` or on any `ArgSpec.type` is rejected at `register()` time with a targeted error. Every transformer must still declare a concrete output type.
- When an Any-input transformer is used in a chain, the input-type equality check is skipped for that step. Downstream strictness is preserved — the next transformer in the chain is still checked against the Any-input transformer's concrete `outputType`. For example, `col|accepts_any_returning_array|upper` still errors at `upper` because `array` is not a valid input to `upper`.
- `parseFlyQLType("any")` raises an error: `any` is not a valid column or value type. It is a transformer-input sentinel only.
...
class TypeTransformer(Transformer):
    ...
    @property
    def input_type(self): return Type.Any      # accepts any upstream type
    @property
    def output_type(self): return Type.String  # must be concrete
    def sql(self, dialect, column_ref, args=None):
        if dialect == "clickhouse":
...
class TypeTransformer extends Transformer {
    ...
    get inputType() { return Type.Any }         // accepts any upstream type
    get outputType() { return Type.String }     // must be concrete
...
func (TypeTransformer) OutputType() flyqltype.Type        { return flyqltype.String }  // concrete
...
Available types: see [`flyql.Type`](/advanced/column-types/). The one exception is `Any`, which is a transformer-input wildcard and not a valid column or value type.
...
- [Custom Renderers](/advanced/custom-renderers/) — register post-alias display
  metadata descriptors. Unlike transformers, renderers never affect generated
  SQL; they only describe how your application should render a value.
