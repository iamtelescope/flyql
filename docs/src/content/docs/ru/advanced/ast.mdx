...
| `value` | `any` | Значение для сравнения. Строка, число, Boolean или Null для скалярных операторов. Пустое для <FlyqlToken type="operator">in</FlyqlToken> и <FlyqlToken type="operator">not in</FlyqlToken>. |
| `valueType` | `string` или `null` | Явный тип `value`: `"integer"`, `"bigint"`, `"float"`, `"string"`, `"boolean"` или `"null"`. `null` для <FlyqlToken type="operator">in</FlyqlToken>/<FlyqlToken type="operator">not in</FlyqlToken>. |
| `values` | `array` или `null` | Список значений для операторов <FlyqlToken type="operator">in</FlyqlToken> и <FlyqlToken type="operator">not in</FlyqlToken>. Может быть гетерогенным. `null` для остальных операторов. |
| `valuesTypes` | `string[]` или `null` | Массив типов для каждого элемента, параллельный `values`. Каждый элемент — одна из констант типов. `null`, если `values` не используется. |
