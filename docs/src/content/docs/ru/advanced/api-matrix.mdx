...
[Токенизатор](./tokenize) — это общий примитив, который группирует посимвольный вывод парсера в типизированные токены формы `{text, type, start, end}`. Используйте его, чтобы строить собственные подсветчики или форматтеры поверх FlyQL.
