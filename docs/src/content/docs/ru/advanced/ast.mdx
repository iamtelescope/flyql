...
| Поле | Тип | Описание |
|------|-----|----------|
| `boolOperator` | `string` | `"and"` или `"or"` — способ объединения `left` и `right`. Пустая строка для листовых узлов. |
...
| `negated` | `boolean` | `true`, если узел обёрнут в `not`. |
...
| Поле | Тип | Описание |
|------|-----|----------|
| `key` | `Key` | Поле, которое проверяется (например, `status`, `user.metadata.role`). |
| `operator` | `string` | Один из: `=`, `!=`, `~`, `!~`, `>`, `<`, `>=`, `<=`, `truthy`, `in`, `not in`, `has`, `not has`. |
| `value` | `any` | Значение для сравнения. Строка, число, Boolean или Null для скалярных операторов. Пустое для `in` и `not in`. |
| `valueType` | `string` или `null` | Явный тип `value`: `"integer"`, `"bigint"`, `"float"`, `"string"`, `"boolean"` или `"null"`. `null` для `in`/`not in`. |
| `values` | `array` или `null` | Список значений для операторов `in` и `not in`. Может быть гетерогенным. `null` для остальных операторов. |
...
Запрос: `status = 200 and method = "GET"`
...
Запрос: `env=prod and (status=500 or level = error)`
...
Запрос: `user.metadata.role = "admin"`
