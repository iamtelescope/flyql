...
| `registry` | `TransformerRegistry` | `null` | Custom transformer registry (see [Custom Transformers](/advanced/custom-transformers/)) |
| `rendererRegistry` | `RendererRegistry` | `null` | Custom renderer registry (see [Custom Renderers](/advanced/custom-renderers/)). When `null` the editor disables renderer parsing entirely; supplying any registry (even an empty one) enables the post-alias `\|` grammar. |
