...
| Column/value type ([`flyql.Type`](./column-types)) | `flyql.Type` (`flyql.TypeString`, ...) | `Type` (`Type.String`, ...) | `Type` (`Type.String`, ...) |
...
The [tokenizer](./tokenize) is a shared primitive that groups the parser's per-character output into typed tokens with `{text, type, start, end}`. Use it to build custom highlighters or formatters on top of FlyQL.
...
Each dialect package exposes a helper that bridges from a list of dialect-specific Columns to a canonical `flyql.ColumnSchema` for use with the validator. See [Column Types](./column-types) for the full contract.
