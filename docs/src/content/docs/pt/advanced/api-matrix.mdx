...
O [tokenizador](/pt/advanced/tokenize/) é uma primitiva partilhada que agrupa a saída carácter a carácter do parser em tokens tipados com a forma `{text, type, start, end}`. Use-o para construir realçadores ou formatadores personalizados sobre o FlyQL.
...
O modo columns é exclusivo do JavaScript. Python lança `ValueError` e Go devolve um erro não-nil com a mensagem `columns mode is only available in the JavaScript package`.
