...
| `valueType` | `string` or `null` | `value` の明示的な型。`"integer"`, `"bigint"`, `"float"`, `"string"`, `"boolean"`, `"null"` のいずれか。<FlyqlToken type="operator">in</FlyqlToken> / <FlyqlToken type="operator">not in</FlyqlToken> では `null` |
