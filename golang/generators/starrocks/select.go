...
func buildSelectExpr(column *Column, path []string) (string, error) {
	...
	if column.JSONString {
		...
		return fmt.Sprintf("parse_json(`%s`)->%s", column.Name, strings.Join(pathParts, "->")), nil
	}
	...
	if column.IsMap {
		...
		return fmt.Sprintf("`%s`[%s]", column.Name, escapedKey), nil
	}
	...
	if column.IsStruct {
		...
		return fmt.Sprintf("`%s`.`%s`", column.Name, structColumn), nil
	}
...
func ToSQLSelect(text string, columns map[string]*Column) (*SelectResult, error) {
	...
	for _, raw := range raws {
		...
		if len(key.Transformers) > 0 {
			...
			sqlExpr, err = applyTransformerSQL(sqlExpr, key.Transformers, "starrocks", registry)
			if err != nil {
				return nil, fmt.Errorf("column %q: %w", raw.name, err)
			}
		}
