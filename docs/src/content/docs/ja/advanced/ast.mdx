...
| フィールド | 型 | 説明 |
|------------|----|------|
| `boolOperator` | `string` | `"and"` または `"or"`。`left` と `right` の結合方法を示します。leaf ノードでは空です |
...
| `negated` | `boolean` | このノードが `not` で包まれている場合は `true` |
...
| フィールド | 型 | 説明 |
|------------|----|------|
| `key` | `Key` | 評価対象のフィールド。例: `status`、`user.metadata.role` |
| `operator` | `string` | `=`, `!=`, `~`, `!~`, `>`, `<`, `>=`, `<=`, `truthy`, `in`, `not in`, `has`, `not has` のいずれか |
| `value` | `any` | 比較値。スカラー演算子では string、number、boolean、null のいずれかです。`in` と `not in` では空です |
| `valueType` | `string` or `null` | `value` の明示的な型。`"integer"`, `"bigint"`, `"float"`, `"string"`, `"boolean"`, `"null"` のいずれか。`in` / `not in` では `null` |
| `values` | `array` or `null` | `in` / `not in` 用の値リスト。異種型も可。他の演算子では `null` |
...
クエリ: `env=prod and (status=500 or level = error)`
