...
def load_columns():
    ...
    for name, fd in data["columns"].items():
        kwargs = {}
        if "raw_identifier" in fd:
            kwargs["raw_identifier"] = fd["raw_identifier"]
        columns[name] = Column(fd["name"], fd["type"], fd.get("values"), **kwargs)
    return columns
...
def test_formatting(columns, case):
    ...
    assert unformatted == case["expected_unformatted_sql"]
    assert formatted == case["expected_formatted_sql"]
    assert "\n" not in unformatted
    assert normalize_ws(formatted) == unformatted
