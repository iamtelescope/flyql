...
为嵌入宿主浮层内的编辑器，提供了对建议面板落点的控制。

新功能：

- **`--flyql-panel-z-index`**（默认 `100`）。面板的层叠位置此前是写死的数值，宿主浮层若在其之上，就只能通过针对 `.flyql-panel` 写规则来解决遮挡。抽屉与模态框通常远高于 100，于是编辑器照常工作，面板却完全不可见。
- **`FlyqlEditor` 与 `FlyqlColumns` 的 `panelContainer` 属性**（默认 `body`，两个包均支持）。接受选择器或元素；面板会挂载到那里而非 `document.body`，并继承该元素的层叠上下文，无需再计算 z-index。无法解析的目标会回退到 `body`，且每次打开面板时都会重新解析，因此晚于编辑器挂载的浮层同样有效。
- **挂载节点上的 `data-flyql-panel` 属性**，让在外部点击时关闭浮层的宿主无需依赖内部类名即可识别面板。设置了 `panelContainer` 时，面板本就是浮层的后代，这项判断也不再需要。

对现有用户没有任何变化：面板仍挂载到 `document.body`，层叠值仍为 100。

## 2026.08.25
版本：**1.3.0**

用于将编辑器嵌入宿主设计系统的接入点：单行模式、清除按钮，以及此前写死在代码里的盒模型尺寸。

新功能：

- **`FlyqlEditor` 与 `FlyqlColumns` 的 `multiline` 属性**（默认 `true`，两个包均支持）。设为 `false` 时字段保持单行：Shift+Enter 不再插入换行，来自粘贴、拖放或输入法的换行会被替换为空格，长查询横向滚动而不换行。
- **`hasClear` 与 `clearButtonLabel` 属性**（默认 `false` 与 `'Clear'`）。字段末端的清除按钮，仅在字段有值时出现。点击后会沿常规的值变更路径清空字段 —— 建议面板关闭、诊断消失，与手动删除完全一致 —— 并把焦点交还给输入框。
- **新增主题变量** `--flyql-border-radius`、`--flyql-padding-block`、`--flyql-label-font-weight` 与 `--flyql-border-hover`，分别对应圆角、垂直内边距、标签字重与悬停边框。默认值均与当前一致，宿主未设置前不会有任何变化。参见[主题](/zh/editor/theming/)。

Bug 修复：

- **起始间距不再随滚动消失。** 它原本位于输入框的 `padding-inline-start`，而横向滚动容器的内边距只在滚动位置 0 处存在，因此滚动后的查询会钻到标签下方。现在左右两侧间距都由不滚动的 `.flyql-<root>__container` 承担。
- **高亮层在滚动到底时不再落后光标一像素。** `width: 100%` 覆盖了它的 `right` 偏移 —— 绝对定位盒中 `width` 优先于 `right` —— 导致两个文本层的滚动范围不一致。
- **`Home` 与 `End` 会同时移动视图。** 两者都在 `preventDefault()` 之后设置选区，这会抑制浏览器自身的滚动到可视区；在可滚动的查询中光标会跑出屏幕，按键看起来毫无反应。

文档：

- `submit` 事件与快捷键表写的是 Shift+Enter，但组件一直是在 Ctrl/Cmd+Enter 时触发。已在全部 11 种语言中更正，并补充了 Shift+Enter 的实际行为。

直接覆盖 `.flyql-<root>__input` 内边距的宿主应重新检查：渲染结果不变，但水平内边距已移到容器上。
