---
title: Matriz de API entre linguagens
description: Mapeamento completo de funções e tipos de API pública em Go, Python e JavaScript
---
...
O [tokenizador](/pt-br/advanced/tokenize/) é uma primitiva compartilhada que agrupa a saída caractere a caractere do parser em tokens tipados com a forma `{text, type, start, end}`. Use-o para construir realçadores ou formatadores customizados sobre o FlyQL.

| Operation | Go | Python | JavaScript |
|-----------|------|--------|------------|
| Tokenize query | `Tokenize(text, "query")` | `tokenize(text)` | `tokenize(text)` |
| Tokenize columns | _(N/A)_ | _(lança `ValueError`)_ | `tokenize(text, { mode: 'columns' })` |
| **Types** | | | |
| Token | `Token` (plain struct) | `Token` (frozen dataclass) | plain object |

O modo columns é exclusivo do JavaScript. Python lança `ValueError` e Go retorna um erro não-nil com a mensagem `columns mode is only available in the JavaScript package`.
