---
title: Cross-Language API Matrix
description: Complete mapping of public API functions and types across Go, Python, and JavaScript
---
...
| Column/value type ([`flyql.Type`](/advanced/column-types/)) | `flyql.Type` (`flyql.TypeString`, ...) | `Type` (`Type.String`, ...) | `Type` (`Type.String`, ...) |
...
The [tokenizer](/advanced/tokenize/) is a shared primitive that groups the parser's per-character output into typed tokens with `{text, type, start, end}`. Use it to build custom highlighters or formatters on top of FlyQL.

| Operation | Go | Python | JavaScript |
|-----------|------|--------|------------|
| Tokenize query | `Tokenize(text, "query")` | `tokenize(text)` | `tokenize(text)` |
| Tokenize columns | _(N/A)_ | _(raises `ValueError`)_ | `tokenize(text, { mode: 'columns' })` |
| **Types** | | | |
...
## Dialect → flyql.ColumnSchema bridges

Each dialect package exposes a helper that bridges from a list of dialect-specific Columns to a canonical `flyql.ColumnSchema` for use with the validator. See [Column Types](/advanced/column-types/) for the full contract.
