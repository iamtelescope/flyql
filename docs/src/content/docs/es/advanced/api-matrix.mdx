---
title: Matriz de API entre lenguajes
description: Mapeo completo de funciones y tipos de API pública en Go, Python y JavaScript
---
...
El [tokenizador](/es/advanced/tokenize/) es una primitiva compartida que agrupa la salida por carácter del parser en tokens tipados con `{text, type, start, end}`. Úsalo para construir resaltadores o formateadores personalizados sobre FlyQL.

| Operation | Go | Python | JavaScript |
|-----------|------|--------|------------|
| Tokenize query | `Tokenize(text, "query")` | `tokenize(text)` | `tokenize(text)` |
| Tokenize columns | _(N/A)_ | _(lanza `ValueError`)_ | `tokenize(text, { mode: 'columns' })` |
| **Types** | | | |
| Token | `Token` (plain struct) | `Token` (frozen dataclass) | plain object |

El modo columns es exclusivo de JavaScript. Python lanza `ValueError` y Go retorna un error no-nil con el mensaje `columns mode is only available in the JavaScript package`.
