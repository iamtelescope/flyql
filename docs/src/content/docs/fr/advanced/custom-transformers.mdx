...
| `inputType` | TransformerType | Type d'entree attendu (`string`, `int`, `float`, `bool`, `array`) |
...
|-------|------|------------|-------------|
| `type` | TransformerType | — | Type d'argument attendu (`string`, `int`, `float`, `bool`) |
...
columns = [Column(name="host", jsonstring=False, _type="String", normalized_type="string")]
diags = diagnose(ast, columns)
# [Diagnostic(range=Range(0, 4), message="column 'hstt' is not defined", severity='error', code='unknown_column')]
```

```javascript
// JavaScript
import { parse, Column, diagnose } from 'flyql'

const ast = parse('hstt|upper=X').root
const columns = [new Column('host', false, 'String', 'string')]
const diags = diagnose(ast, columns)
// [Diagnostic { range: Range(0, 4), message: "column 'hstt' is not defined", severity: 'error', code: 'unknown_column' }]
```
...
columns := []flyql.Column{flyql.NewColumn("host", false, "String", "string")}
diags := flyql.Diagnose(result.Root, columns, nil) // nil = default registry
