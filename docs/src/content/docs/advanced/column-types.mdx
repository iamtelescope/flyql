...
`Type.Any` (string value `"any"`) is a **sentinel used only as a transformer's input type**. It declares that the transformer accepts any upstream column or transformer output type, regardless of what that type is. It is intentionally **not** one of the column-eligible values in the table above:

- `parseFlyQLType("any")` / `ParseType("any")` raises an error. The strict parser rejects `"any"` as a column type token.
- `TransformerRegistry.register` rejects any transformer that declares `Any` as `output_type` or on any `ArgSpec.type`. Every transformer must still declare a concrete output type; `Any` on output would break downstream chain typing.

When an AST validator, columns validator, or runtime chain helper encounters a transformer with `input_type == Any`, the input-type equality check is skipped for that step. Downstream strictness is preserved — the next transformer is still checked against the Any-input transformer's concrete `output_type`, so `col|accepts_any_returning_array|upper` still errors at `upper` because `array` is not a valid input to `upper`.

See [Custom transformers — Any-input transformers](/advanced/custom-transformers/#any-input-transformers) for declaration and usage.
