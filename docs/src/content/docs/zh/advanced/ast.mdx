...
| 字段 | 类型 | 说明 |
|------|------|------|
| `key` | `Key` | 被测试的字段（例如 `status`、`user.metadata.role`）。 |
| `operator` | `string` | 以下之一：`=`、`!=`、`~`、`!~`、`>`、`<`、`>=`、`<=`、`truthy`、`in`、`not in`、`has`、`not has`。 |
| `value` | `any` | 比较值。对于标量操作符，可以是字符串、数字、Boolean 或 Null。对于 `in` 和 `not in` 为空。 |
| `valueType` | `string` or `null` | `value` 的显式类型：`"integer"`、`"bigint"`、`"float"`、`"string"`、`"boolean"` 或 `"null"`。`in`/`not in` 时为 `null`。 |
| `values` | `array` or `null` | `in` 和 `not in` 操作符的值列表。可以是异构的。其他操作符时为 `null`。 |
...
查询：`env=prod and (status=500 or level = error)`
