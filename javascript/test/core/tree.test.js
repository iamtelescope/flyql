...
import { Node, Expression, FlyqlError, VALID_BOOL_OPERATORS } from '../../src/index.js'

const INVALID_OPERATOR_VALUE = 'INVALID_OPERATOR'

function getValidExpression() {
    return new Expression('a', '=', 'b', null)
}

function getValidNode() {
    return new Node(
        VALID_BOOL_OPERATORS[0],
        getValidExpression(),
        null,
        null
    )
...
describe('Node', () => {
    ...
    it('should throw error for invalid bool operator', () => {
        const node = getValidNode()
        expect(() => node.setBoolOperator(INVALID_OPERATOR_VALUE))
            .toThrow(FlyqlError)
    })
    ...
    it('should throw error when both children and expression are provided', () => {
        ...
        expect(() => new Node('and', expression, left, right))
            .toThrow(FlyqlError)
    })
})