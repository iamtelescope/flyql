---
title: Schema 配置
description: FlyQL 编辑器中的列 schema 结构
---

`columns` prop 用来定义驱动编辑器建议与校验的数据 schema。键名是列名，值是对应的列定义。
...
固定值集合列。和 `number` 一样，不支持正则建议。通常通过 `values` 提供候选项：
...
如果设置 `autocomplete: true` 但不提供 `values`，则需要在编辑器组件上传入 `onAutocomplete`：
...
如果对象列的嵌套字段无法在 schema 中提前定义，可以省略 `children`，并通过 `onKeyDiscovery` 动态发现：
