...
| `value` | `any` | The comparison value. A string, number, boolean, or null for scalar operators. Empty for `in` and `not in`. |
| `valueType` | `string` or `null` | The explicit type of `value`: `"integer"`, `"bigint"`, `"float"`, `"string"`, `"boolean"`, or `"null"`. `null` for `in`/`not in`. |
| `values` | `array` or `null` | List of values for `in` and `not in` operators. Can be heterogeneous. `null` for other operators. |
