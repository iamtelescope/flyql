...
2. {/* TODO: native-speaker review */} **木を再帰的に走査する：** branch ノードは `left OP right` として結合され、SQL の優先順位が要求する場合にのみ括弧を付けます（例: `AND` の親の下にある `OR` サブツリーは `a AND (b OR c)` として保持され、`a AND b AND c` のような同一優先度のチェーンはフラットに出力されます）。`NOT (...)` は SQL の慣習に従い、常にそのオペランドを包みます。leaf ノードは SQL 条件になります。
