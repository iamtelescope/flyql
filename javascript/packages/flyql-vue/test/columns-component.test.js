...
describe('FlyqlColumns component', () => {
    ...
    describe('ARIA accessibility (AC #6)', () => {
        it('textarea has aria-label', () => {
            ...
            expect(vueContent).toContain("'FlyQL columns expression input'")
        })
...
describe('prefix slot (icon + label)', () => {
    ...
    it('renders icon and label inside a single prefix element', () => {
        expect(vueContent).toContain('class="flyql-columns__prefix"')
        expect(vueContent).toContain('class="flyql-columns__icon"')
        expect(vueContent).toContain('class="flyql-columns__label"')
    })
    ...
    it('lets a visible label be the accessible name of the input', () => {
        expect(vueContent).toContain(':aria-label="inputAriaLabel"')
        expect(vueContent).toContain(
            "const inputAriaLabel = computed(() => props.label || 'FlyQL columns expression input')",
        )
    })
})
...
describe('single-line mode (`multiline`)', () => {
    ...
    it('marks the root so the CSS can stop the text wrapping', () => {
        expect(vueContent).toContain("'flyql-columns--single-line': !multiline,")
    })
...
describe('clear button (`hasClear`)', () => {
    ...
    it('sits outside the scrolling input, as a sibling of the container', () => {
        ...
        const button = vueContent.indexOf('class="flyql-columns__clear"')
        const container = vueContent.indexOf('class="flyql-columns__container"')
