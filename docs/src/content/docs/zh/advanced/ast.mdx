...
| `valueType` | `string` or `null` | `value` 的显式类型：`"integer"`、`"bigint"`、`"float"`、`"string"`、`"boolean"`、`"null"`、`"function"` 或 `"parameter"`。<FlyqlToken type="operator">in</FlyqlToken>/<FlyqlToken type="operator">not in</FlyqlToken> 时为 `null`。 |
