...
describe('single-line mode (`multiline`)', () => {
    ...
    it('marks the root so the CSS can stop the text wrapping', () => {
        expect(jsxContent).toContain("(multiline ? '' : ' flyql-editor--single-line')")
    })
...
describe('Shift+Enter in single-line mode', () => {
    it('is blocked rather than inserting a break', () => {
        expect(jsxContent).toContain('if (!multiline) {')
        expect(jsxContent).toContain('e.preventDefault()')
    })
})
...
describe('clear button (`hasClear`)', () => {
    ...
    it('sits outside the scrolling input, as a sibling of the container', () => {
        ...
        const button = jsxContent.indexOf('className="flyql-editor__clear"')
        const container = jsxContent.indexOf('className="flyql-editor__container"')
