...
- **Columns mode is JavaScript-only.** The Python columns parser has no typed-character output and there is no Go columns parser.
...
- [Cross-Language API Matrix](./api-matrix) — the canonical function name for each language.
...
- [AST & Custom Generators](./ast) — the higher-level tree you get from `parse()`. `tokenize()` is a complement, not a replacement: it gives you a flat view where `parse()` gives you a tree.
