...
O flyql fornece um único registo de todos os códigos de diagnóstico e errnos do parser. O registo é a fonte da verdade que mantém Python, JavaScript e Go sincronizados, e cada `Diagnostic` ou erro do parser que um consumidor vê transporta uma referência opcional para ele.
