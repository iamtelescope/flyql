...
Le [tokeniseur](./tokenize) est une primitive partagée qui regroupe la sortie caractère-par-caractère du parser en tokens typés de forme `{text, type, start, end}`. Utilise-le pour construire des colorisateurs ou formateurs personnalisés au-dessus de FlyQL.
