...
|------|-----|--------------|
| `boolOperator` | `string` | `"and"` oder `"or"` – bestimmt, wie `left` und `right` kombiniert werden. Bei leaf-Knoten leer |
...
| `negated` | `boolean` | `true`, wenn dieser Knoten in `not` eingeschlossen ist |
...
| Feld | Typ | Beschreibung |
|------|-----|--------------|
| `key` | `Key` | Das geprüfte Feld (z. B. `status`, `user.metadata.role`) |
| `operator` | `string` | Einer von: `=`, `!=`, `~`, `!~`, `>`, `<`, `>=`, `<=`, `truthy`, `in`, `not in`, `has`, `not has` |
| `value` | `any` | Der Vergleichswert. Ein String, eine Zahl, ein Boolean oder null für skalare Operatoren. Leer bei `in` und `not in` |
| `valueType` | `string` oder `null` | Der explizite Typ von `value`: `"integer"`, `"bigint"`, `"float"`, `"string"`, `"boolean"` oder `"null"`. `null` für `in`/`not in` |
| `values` | `array` oder `null` | Werteliste für `in` und `not in`. Kann heterogen sein. `null` bei anderen Operatoren |
...
Abfrage: `status = 200 and method = "GET"`
...
Abfrage: `env=prod and (status=500 or level = error)`
...
Abfrage: `user.metadata.role = "admin"`
