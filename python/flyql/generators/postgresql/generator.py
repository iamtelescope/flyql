...
def _walk_where(
    ...
    if left_text and right_text:
        ...
        sql_bool_op = BOOL_OP_TO_SQL[parent_op]
        if opts.format and (
            "\n" in left_sql or "\n" in right_sql or left_op or right_op
        ):
            text = f"{left_sql}\n{sql_bool_op} {right_sql}"
        else:
            text = f"{left_sql} {sql_bool_op} {right_sql}"
...
def to_sql_where_with_options(
    ...
    *,
    ...
    """Returns PostgreSQL WHERE clause honoring ``options`` (formatting + tz)."""
    text, _ = _walk_where(
        root,
        columns,
        registry=registry,
...
def to_sql_select_with_options(
    ...
    *,
    ...
    """Generate a PostgreSQL SELECT clause honoring ``options`` (formatting)."""
